...
std::string outputname_from_settings_crown(std::string input, std::string folder, unsigned int first_entry, unsigned int last_entry, fs::path output_dir="", std::string era="2018", std::string channel="tt", bool organize_outputs = true)
{
    std::cout << "Using input " << input << " folder " << folder << " first entry " << first_entry << " last entry " << last_entry << " era " << era << " channel " << channel << std::endl;
    std::string filename = filename_from_inputpath(input);
    fs::path full_path = "";
    if(organize_outputs) {
        std::string outputname = filename + "/" + filename + "_" + folder + "_" + era + "_" + channel + "_" + std::to_string(first_entry) + "_" + std::to_string(last_entry) + ".root";
        full_path = output_dir / outputname;
    }
    else {
        std::string outputname = filename + "_" + folder + "_" + era + "_" + channel + "_" + std::to_string(first_entry) + "_" + std::to_string(last_entry) + ".root";
        full_path = outputname;
    }
    return full_path.string();
}
...
std::map<std::string, std::vector<std::string>> build_quantities_map(std::vector<std::string> quantities)
{
    // first, loop over all quantities and split the string by __ to get all pipelines
    ...
    for(auto quantity : quantities) {
        ...
        if (pipeline_split.size() < 2) {
            std::cout << "Warning: quantity " << quantity << " does not contain a pipeline, cont." << std::endl;
            continue;
        }
